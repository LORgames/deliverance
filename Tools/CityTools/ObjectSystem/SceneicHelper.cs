...
namespace CityTools.ObjectSystem {
    public class SceneicHelper {
        ...
        public static void MouseUp(MouseEventArgs e) {
            ...
            Box2D.B2System.world.QueryAABB(new Box2CS.World.QueryCallbackDelegate(SceneicHelper.QCBD), aabb);
        ...
        private static int FindObjectIndex() {
            ...
            for (int i = 0; i < ScenicObjectCache.s_objectStore.Count; i++) {
                if (currentIndex == ScenicObjectCache.s_objectStore[i].index) {
                    listIndex = i;
                }
            }
        ...
        internal static void SendBack() {
            ...
            ScenicObjectCache.s_objectStore.Sort();

            int listIndex = FindObjectIndex();

            if (listIndex != 0) {
                // Swap values
                int temp = ScenicObjectCache.s_objectStore[listIndex].index;
                ScenicObjectCache.s_objectStore[listIndex].index = ScenicObjectCache.s_objectStore[listIndex - 1].index;
                ScenicObjectCache.s_objectStore[listIndex - 1].index = temp;
        ...
        internal static void BringForward() {
            ...
            if (listIndex != ScenicObjectCache.s_objectStore.Count - 1) {
                // Swap values
                int temp = ScenicObjectCache.s_objectStore[listIndex].index;
                ScenicObjectCache.s_objectStore[listIndex].index = ScenicObjectCache.s_objectStore[listIndex + 1].index;
                ScenicObjectCache.s_objectStore[listIndex + 1].index = temp;
        ...
        internal static void SendToBack() {
            ...
            for (int i = listIndex; i > 0; i--) {
                int temp = ScenicObjectCache.s_objectStore[i].index;
                ScenicObjectCache.s_objectStore[i].index = ScenicObjectCache.s_objectStore[i - 1].index;
                ScenicObjectCache.s_objectStore[i - 1].index = temp;
        ...
        internal static void BringToFront() {
            ...
            for (int i = listIndex; i < ScenicObjectCache.s_objectStore.Count - 1; i++) {
                int temp = ScenicObjectCache.s_objectStore[i].index;
                ScenicObjectCache.s_objectStore[i].index = ScenicObjectCache.s_objectStore[i + 1].index;
                ScenicObjectCache.s_objectStore[i + 1].index = temp;
